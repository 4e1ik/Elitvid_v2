    ...
    <div id="content">
        <div class="panel box-shadow-none content-header">
            <div class="panel-body">
                <div class="col-md-12">
                    <h3 class="animated fadeInLeft">Список скамеек коллекции Street Furniture</h3>
                </div>
                                ...
                                @if($products->isNotEmpty())
                                    @foreach($products as $product)
                                        <tr>
                                            ...
                                            <td class="admin-table-cell-truncate admin-table-cell-truncate--short" title="{{ $product->bench->material ?? '' }}">{{ Str::limit($product->bench->material ?? '', 30) }}</td>
                                            <td>
                                                @if($product->bench->data && is_array($product->bench->data) && count($product->bench->data) > 0)
                                                    <button type="button" 
                                                        ...
                                                        Показать варианты ({{count($product->bench->data)}})
                                                    </button>
                                                    <div class="collapse" id="variants-{{$product->id}}">
                                                        <div class="variants-list" style="margin-top: 5px;">
                                                            @foreach($product->bench->data as $index => $variant)
